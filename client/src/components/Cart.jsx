			...
			<div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6 gap-3 relative">
				{cart && cart?.length > 0 ? (
					<>
						...
						<div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-6 gap-5">
							<div className="w-full flex items-center justify-evenly">
								<p className="text-3xl text-zinc-500 font-semibold">
									Total
								</p>
								<p className="text-3xl text-red-400 font-semibold flex items-center justify-center gap-1">
									₹ {total}
								</p>
							</div>
							<motion.button
...
export const CartItemCard = ({ index, data }) => {
	...
	const decrementCart = (productId) => {
		dispatch(alertSuccess("Decreased the quantity of the item"));
	...
	const incrementCart = (productId) => {
		dispatch(alertSuccess("Increased the quantity of the item"));
	...
	return (
		...
		>
			...
			<div className="flex items-center justify-start gap-1 w-full">
				...
				<p className="text-sm flex items-center justify-center font-semibold text-red-400 ml-auto gap-1">
					₹ {itemTotal}
				</p>
			</div>
